...
const StepFormContextProvider = ({
  ...
  const api = useMemo(
    () => ({ activeStep, setActiveStep, onNext, onBack, steps, addStep }),
    [activeStep, addStep, onBack, onNext, steps]
  );

  return (
    <StepFormContext.Provider value={api}>{children}</StepFormContext.Provider>
  );
};
